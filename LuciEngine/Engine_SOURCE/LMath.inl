...
inline lu::math::Rectangle Rectangle::Intersect(const Rectangle& ra, const Rectangle& rb) noexcept
{
...
inline lu::math::Rectangle Rectangle::Union(const Rectangle& ra, const Rectangle& rb) noexcept
{
    const long righta = ra.x + ra.width;
    const long rightb = rb.x + rb.width;
...
inline bool Ray::operator == (const Ray& r) const noexcept
{
    ...
    const XMVECTOR r1d = XMLoadFloat3(&velocity);
    const XMVECTOR r2d = XMLoadFloat3(&r.velocity);
...
inline bool Ray::Intersects(const Plane& plane, _Out_ float& Dist) const noexcept
{
    ...
    const XMVECTOR p = XMLoadFloat4(&plane);
    const XMVECTOR dir = XMLoadFloat3(&velocity);
